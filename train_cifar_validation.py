...
flags.DEFINE_string('logdir', './log/cifar_val', 'log directory')
flags.DEFINE_integer('seed', 10, 'seed numpy')
flags.DEFINE_integer('seed_data', 10, 'seed data')
flags.DEFINE_integer('labeled', 300, 'labeled data per class')
flags.DEFINE_integer('validation', 100, 'validation per class')
...
def main(_):
    ...
    for j in range(10):
        ...
        validationx.append(trainx[trainy == j][FLAGS.labeled:FLAGS.labeled + FLAGS.validation])
        validationy.append(trainy[trainy == j][FLAGS.labeled:FLAGS.labeled + FLAGS.validation])
    txs = np.concatenate(txs, axis=0)
    tys = np.concatenate(tys, axis=0)
    validationx = np.concatenate(validationx, axis=0)
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop():
            ...
            begin = time.time()
            train_loss_lab= train_loss_unl= train_loss_gen= train_acc= test_acc= test_acc_ma= train_j_loss=val_acc_ma=val_acc = 0
